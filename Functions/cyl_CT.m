function [NU, CV, HU, slice_flags, positions] = cyl_CT(image, dicom_headers)
...
mascara_small = medfilt2(medias) > -10;
...
%% HU
figure;
plot( positions(indices), HU(indices), 'k -');
%ylim([-10 10]);

HUs_ok = HU(indices);
media_HU = mean(HUs_ok(:));
max_HU = max(HUs_ok(:));
std_HU = std(HUs_ok(:));
min_HU = min(HUs_ok(:));

hold on;
plot(positions(indices), media_HU,'k --');

posi = positions(indices);

title( ['HU ' fecha(7:8) '/' fecha(5:6) '/' fecha(1:4)  ] )
xlabel('Posición Axial [mm]');
ylabel('HU');

f = '%6.2g';
text(posicion_inicio+10, 9, ['HU: ' num2str(media_HU,f) ' +- ' num2str(std_HU,f) '  [' num2str(min_HU,f) '-' num2str(max_HU,f) ']  %'])
disp(['HU: ' num2str(media_HU,f) ' +- ' num2str(std_HU,f) '  [' num2str(min_HU,f) '-' num2str(max_HU,f) ']  %']);
...
title( ['Uniformidad Tomográfica ' fecha(7:8) '/' fecha(5:6) '/' fecha(1:4)  ] )
xlabel('Posición Axial [mm]');
...
title( ['Coeficientes de Variación ' fecha(7:8) '/' fecha(5:6) '/' fecha(1:4)  ] )
xlabel('Posición Axial [mm]');
ylabel('CV [%]');
...
hold on
plot( positions(indices), HU(indices), 'b -' );
hold on

legend('Slice Non-uniformity [%]','Slice Coefficient of variation [%]', 'Slice HU')
...
text(pos1+10, -10,['HU: ' num2str(media_HU,f) ' +- ' num2str(std_HU,f) '  [' num2str(min_HU,f) '-' num2str(max_HU,f) ']  %']);
